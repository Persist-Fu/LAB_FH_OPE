...
void InternalNode::insert_node(int index, Node * new_node)
{
    ...
    {
        //如果新节点是 LeafNode
        this->child_num.insert(this->child_num.begin() + index,
((LeafNode *)new_node)->cipher.size());
    ...
    //插入的新 node 改变了原有 node 位置,因此需要重新记录 parent_index 和
    //child_num
...
{
    int old_child_count = (int)this->child.size();
    fprintf(stderr, "[SPLIT] InternalNode rebalance: children=%d (threshold M=%d)\n",
            old_child_count, M);

    InternalNode * new_node = new InternalNode();
    //将当前节点的后半部分数据存入 new_node
    ...
    if (!this->parent)
    {
        fprintf(stderr, "[SPLIT] InternalNode rebalance: promoted new root (split %d -> %d + %d children)\n",
                old_child_count, (int)this->child.size(), (int)new_node->child.size());
        //如果当前节点是 root 节点
    ...
    {
        fprintf(stderr, "[SPLIT] InternalNode rebalance: split %d -> %d + %d children under parent\n",
                old_child_count, (int)this->child.size(), (int)new_node->child.size());
        int res = 0;
...
void Recode(vector<LeafNode *> node_list)
{
    ...
    if ((right_bound - left_bound) > total_cipher_num)
    {
        ...
        fprintf(stderr,
                "[RECODE] encoding updated: range [%lld, %lld), leaf_nodes=%zu, ciphers=%d, remapped=%zu\n",
                start_update, end_update, node_list.size(), total_cipher_num, update.size());
    }
    else
    {
        fprintf(stderr,
                "[RECODE] interval insufficient: range [%lld, %lld), ciphers=%d, leaf_nodes=%zu; expanding...\n",
                left_bound, right_bound, total_cipher_num, node_list.size());
        //若不足以包含,则继续向左兄弟节点和右兄弟节点扩展更新区间
...
long long LeafNode::Encode(int pos)
{
    ...
    {
        fprintf(stderr,
                "[RECODE] triggered at insert pos=%d: interval [%lld, %lld) exhausted (width=%lld)\n",
                pos, left, right, (long long)floor(right - left));
...
{
    int old_cipher_count = (int)this->cipher.size();
    fprintf(stderr, "[SPLIT] LeafNode rebalance: ciphers=%d (threshold M=%d)\n",
            old_cipher_count, M);

    LeafNode * new_node = new LeafNode();
    //将本节点后半部分数据放到 new_node
    ...
    if (!this->parent)
    {
        fprintf(stderr, "[SPLIT] LeafNode rebalance: promoted new root (split %d -> %d + %d ciphers)\n",
                old_cipher_count, (int)this->cipher.size(), (int)new_node->cipher.size());
        //如果 this 是 root 节点,则创建一个新的 root
        InternalNode * new_root = new InternalNode();
        new_root->insert_node(0, this);
        new_root->insert_node(1, new_node);
        root = new_root;
    ...
    {
        fprintf(stderr, "[SPLIT] LeafNode rebalance: split %d -> %d + %d ciphers under parent\n",
                old_cipher_count, (int)this->cipher.size(), (int)new_node->cipher.size());
